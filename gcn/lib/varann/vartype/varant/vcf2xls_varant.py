...
URLS = {'OMIMID': '"http://omim.org/entry/%s";"%s"',
		'Gene_Name':'"http://www.genecards.org/cgi-bin/carddisp.pl?gene=%s";"%s"',
		'OMIM_Ids': '"http://omim.org/entry/%s";"%s"',
		'Transcript':'"https://www.ncbi.nlm.nih.gov/nuccore/%s";"%s"'
		}
...
VCFREM = ['ABHet', 'ABHom', 'AC', 'AF', 'AN', 'Agilent3p', 'Agilent5p',
		  ...
		  'set', 'NEGATIVE_TRAIN_SITE', 'POSITIVE_TRAIN_SITE', 'VARANT_GENIC',
		   'VARANT_INTERGENIC', 'ReagentCapture3p', 'ReagentCapture3p']

HYPER = xlwt.easyxf('font:underline single;font:colour blue')

IN_TRANS = ['trans_id', 'region', 'splice', 'utr_signal', 'cdna','exon', 'mutation', 'codon',
			'aa', 'sift', 'pp2', 'all_the_other_trans']
IN_VAR = ['MIM_IDS', 'MIM_PHENS', 'GAD_PHENS']

IN_EFF = ['GWASPhenotype', 'GerpRSScore', 'DB137', 'KGDB', 'KGAF',
		  'ESPDB', 'ESPAF', 'EXACDB', 'EXACAF', 'IMHQ', 'IMLQ']
IN_TRANS_HDRS = ['Transcript', 'Region', 'SpliceSite','UTR_SIGNAL', 'CDNAPOS', 'Exon', 'Mutation',
				 'Codon Change', 'Amino Acid Change', 'SIFT', 'Polyphen', 'All The Other Transcripts']
...
def write_varant_links(row, rec, vadict, samples, formkeys, cols,
	...
	if genescores:
		score = [float(i[1]) for i in genescores if i[0] == Gene_Name]
		if score:
			if vknown==1 and 'c' in rec['info'].CLASS_TAG:
				score[0] = score[0] + 1.
			row.set_cell_number(tmpidx, score[0])
		else:
			row.set_cell_number(tmpidx, 0.)
		tmpidx += 1
	else:
		pass
	...
	url = URLS['Transcript']
	tx2s = []
	for t in range(len(vadict['TRANSCRIPT'])):
		tx2 = []
		for field in IN_TRANS:
			if field in vadict['TRANSCRIPT'][t]._fields:
				temp = getattr(vadict['TRANSCRIPT'][t], field)
				if field in ['sift', 'pp2']:
					...
					if t == 0:
						...
						tx2.append(temp)
		if tx2:
			tx2s.append('|'.join(tx2))

	#TODO: to add column header for extra TX
	if tx2s:
		row.set_cell_text(tmpidx,';'.join(tx2s))
	else:
		row.set_cell_text(tmpidx,'.')
	tmpidx += 1
	url = URLS['OMIM_Ids']
	for eff in IN_VAR:
		e = vadict[eff]
		if isinstance(e, list):
			if eff == 'MIM_IDS' and len(e)==1 and e[0].strip():
				row.write(tmpidx, xlwt.Formula('HYPERLINK(' + \
					url % (e[0], e[0]) + ')'), HYPER)
			else:
				f = ','.join(e)
...
def run(infile, outfile, hdrfile, genefile, vknown, samples=None, fl=[[], [], [], []]):
	...
	for rec in parser:
		...
		idx = write_varant_links(row, rec, ga, samples, formkeys,
								 colnames, idx, vknown, genescores)
		senum += 1
...
if __name__ == "__main__":
	...
	parser.add_argument('-o', dest='outfile', default=None,
						 help='output file')
	parser.add_argument('-l', dest='logdir', default=None,
						 help='log directory')
	...
	parser.add_argument('-k', dest='vknown', default=0, type=int,
						 help='boost known pathogenic variants in the rank [0:No],1:Yes')
	...
	if options.logdir is None:
		logdir = os.path.join(os.path.dirname(options.infile),'logs')
	else:
	...
	run(options.infile, options.outfile, options.hdrfile,
		 options.genefile, options.vknown, options.samples, [[], [], [], []])
